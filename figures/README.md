...
`official/<edition>/` contains the twelve v0.2.1 SVGs generated by the
reproducible pipeline. Their full-word filenames contain `match-exposure` or
`suspension-exposure`.

`summary/` contains one long presentation image per edition (KPI tiles, the
knockout-team `e_s` ranking, and both winner-perspective differentials, with
the frozen rules and provenance in the footer). They are rendered from the
committed result CSVs by `pipeline/render_edition_summaries.py` via headless
Chrome and are a presentation layer: they are not part of the deterministic
CI gate, and the 2026 image is marked provisional until M101–M104 are
ingested.

Each knockout match-exposure figure takes the FIFA-recorded winner's
perspective (`winner − opponent`) and orders matches by `ΔE_m` from highest to
lowest. Shoot-out winners remain the winner for this display convention. The
third-place fixture is excluded under the methodology's match-exposure scope.

`knockout-match-exposure-per-foul-difference.svg` reports `ΔE_m′`: winner
`E_m / match fouls` minus opponent `E_m / match fouls`, also ordered from
highest to lowest. Numerically this is the existing `Δe_m` field; it is not
divided by fouls twice.
